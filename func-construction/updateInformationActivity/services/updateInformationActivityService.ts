...
@injectable()
class UpdateInformationActivityService {
    ...
    public async execute(payload: IRequestInformationActivity, id: number, accessToken: string): Promise<any> {
        try {
            await this.handleContent.getUser(
                accessToken,
                payload.position,
                DataTypeGetUser.information.entity,
                DataTypeGetUser.action.update
            );

            await this.informationActivityRepository.updateInformationActivity(id, payload);
...
export default UpdateInformationActivityService;